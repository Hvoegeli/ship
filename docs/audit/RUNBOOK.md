...
| 7 Accessibility | `node scripts/audit/cat7-a11y.mjs before` **and** `node scripts/audit/cat7-lighthouse.mjs before` (3×/page median; commits JSON+HTML to `reports/a11y/before/`) **and** `node scripts/audit/cat7-sr-tree.mjs before` (Q3-C SR-tree proxy via CDP `Accessibility.getFullAXTree`, **+ dismissal experiment**: Escape-dismiss test + modal-removed re-snapshot) | `cat7-before.txt`, `cat7-lighthouse-before.txt`, `cat7-sr-before.txt`, `reports/a11y/before/*.report.{json,html}` | `9fe25cd` / `8419aa7` / `cf6a22c` |
...
| #5 timeline | Wednesday MVP submission due **Wednesday MIDNIGHT** (user corrected from "noon" 2026-05-20; working = end of Wed 23:59 CT); early Fri; final Sun | user | the two brief PDFs disagreed; user resolved | ✅ recorded |
| #6 MVP scope | audit-only (detailed brief) vs audit + first fixes (deck slide 9) | user → **audit-only** (2026-05-20) | keeps the frozen audit at zero risk for the hard gate; all 7 fixes are Phase 2 (Fri/Sun). No app-code changes for the Wednesday MVP. | ✅ resolved |
...
| LH-guide Q3 | Screen-reader smoke test: A document-gap / B manual VoiceOver / **C a11y-tree proxy** / C+B both | user → **C + B** | C reproducible (committed script); B adds the qualitative flow check | 🔵 C ✅ (`cf6a22c`/`0d277b1`); B attempted, **blocked (mix of all three: VoiceOver setup friction + app hard to traverse non-visually + tool learning-curve)** → user chose **Path 1+3**: extend C to mechanize the reproducible part + honestly document the flow gap. Extended harness adds the **dismissal experiment** (commit below). |
| Q3-C dismissal experiment + **finding correction** | After extending `cat7-sr-tree.mjs` to press Escape + remove the modal DOM-only and re-snapshot, the data **corrected an earlier overstatement**. | me (caught my own instrument, like Probe 7) | Earlier draft: "keyboard nav broken / inescapable trap / core 508" (H4) + "SR landmark/heading erasure" (H9). Dismissal experiment showed: **`Escape` dismisses on all pages (WCAG 2.1.2 PASSES — not a trap)**; with the modal removed every page exposes **4 landmarks + a proper `h1`** (view_document 19 headings) → **structure is sound; the auto-modal merely occludes it.** Merged H4+H9 → **one High** (auto-modal occlusion, escapable but hits every navigation) + added a **positive** finding (good a11y bones). README verdict now rests on contrast + ARIA, not a keyboard-trap claim. | ✅ AUDIT_REPORT Cat 7 metric rows, verdict, Weaknesses #1–#3, H4 (H9 removed) all corrected |
| Q3 live walkthrough — *interactive a11y-tree pass on the running app* | User asked for a real per-page screen-reader-style walkthrough to catch anything missed. I cannot run VoiceOver (no audio), so I drove the live local URL with Playwright + read the accessibility tree page-by-page (same data an SR consumes), interactively (dismiss modal, tab, inspect editor). | me, per user request | Findings: **(corrections)** login tab order is actually correct (Email→Password→Sign in; the "Password before Email" Low was a misread) — and the occluding dialog is a *visible* Radix modal (the overdue-standup nudge), aria-hides main+nav, `Escape`/`Got it` restores (19→11 aria-hidden); it only appears when the user owes an accountability item. **(positives missed by the harness)** app-wide `Skip to main content`→`#main-content`; proper ARIA `tree`/`treeitem` doc list. **(new)** TipTap editor has no role/aria-label; view_document + my_week page titles are non-descriptive `Ship \| Ship` (WCAG 2.4.2). Evidence screenshot: `reports/a11y/before/cat7-docs-modal.png`. | ✅ folded into AUDIT_REPORT Cat 7 (metric rows + Weaknesses #1/#2/#6/#7/#8) + findings-summary |
| Q3 methodology — *why axe-only is insufficient* | A peer proposed "axe's name/role/structure rules already cover SR concerns — manual/AT pass is redundant." | me → recorded | **axe was *correct* about the static markup** (`main/nav` + `h1` present) **but blind to a runtime defect:** the auto-modal occludes that structure from the SR until dismissed. axe-only would have rated every authed page clean and **never surfaced the auto-modal at all** (runtime behavior, not a markup rule). The markup-layer vs runtime-layer **delta is the finding.** Correct model = 3-tier: axe (names/ARIA) → AT-snapshot + modal-open/removed diff (`cat7-sr-tree.mjs`) → manual VoiceOver (flow). | ✅ AUDIT_REPORT Cat 7 "Methodology note" callout |
...
| Pre-submission readiness sweep (Wed MVP deadline) | Read-only consistency pass across all docs: 28 SHAs all resolve, internal links all resolve, condition-of-record + every per-category metric agree between `AUDIT_REPORT.md` and `findings-summary.html`. Found + fixed **5 doc-only discrepancies**: D1 S8/S11 missing from the ranked table (added → S1–S15 now complete); D2 S12 severity (→ **High** everywhere); D3 S6 (→ **Medium**); D4 S13 (→ **Medium**) — "Medium-High" tier collapsed to the table's High/Medium vocabulary; D5 Cat-1 baseline SHA `44f55d6`(scaffold)→`fe4b76e`(actual cat1 baseline). PDF regenerated (S12 label) + Desktop-mirrored. | me + read-only QA agent → user (severity collapse: "as you'd recommend") | graded artifact; internal inconsistency erodes trust in every number | ✅ this commit |
...
0d277b1  #Q3-C integrate SR-tree findings into AUDIT_REPORT Cat 7 + Ranked Findings
957cded  #Q3 RUNBOOK: C+B decision, cat7-sr-tree in repro table
a7deeaa  #Q3-C findings-summary SR-tree row + PDF
1c5ab28  #Q3 option-3 methodology callout (axe-vs-runtime-AT) in AUDIT_REPORT + RUNBOOK
0a715ee  #Q3 Path-1+3: extend cat7-sr-tree dismissal experiment; CORRECT H4/H9 (escapable, merge→one High + positive); reframe verdict + methodology; record B blockers
<this>   #Q3 live interactive a11y-tree walkthrough: tab-order correction, skip-link/tree positives, editor-label + page-title findings, modal screenshot
```
...
- ✅ **LH-guide Q3** — screen-reader: **C done, B substituted via Path 1+3.** C (`cat7-sr-tree.mjs`) committed; extended with the dismissal experiment, which **corrected an overstated finding** (the modal is escapable; merged H4+H9 → one High + a positive finding). B (manual VoiceOver) attempted, blocked by a mix of setup friction + app difficulty + learning-curve; its reproducible portion is now automated, and the *qualitative announcement-flow* judgment is the only remaining Phase-2 follow-up. **Phase-1 Q3 is closed.**
