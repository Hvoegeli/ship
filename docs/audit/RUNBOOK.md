...
| LH-guide Q3 | Screen-reader smoke test: A document-gap / B manual VoiceOver / **C a11y-tree proxy** / C+B both | user → **C + B** (strongest answer before Wed-noon) | C is reproducible (committed script, fits the pattern of every other Cat); B adds the qualitative SR-flow check automated tools can't substitute for | 🔵 C ✅ committed `cf6a22c` + integrated into AUDIT_REPORT Cat 7 (`0d277b1`); **B attempted, hit blockers (see Q3-B note below) — deferred; AUDIT_REPORT records Tiers 1+2 complete, Tier-3 flow flagged as a remaining gap** |
| Q3 methodology — *why axe-only was rejected* | A peer proposed "axe-core's name/role/structure rules already cover SR concerns (names, ARIA, landmarks, heading order) — manual/AT pass is redundant." | me → recorded as a methodology limitation | **Our own data refutes it for 2 of 4 properties:** axe reported `main/nav=true` + `h1=1` on every authed page, but the runtime a11y tree (`cat7-sr-before.txt`) exposes **zero landmarks on 4/6 pages** and **only the modal's `h2` (no page `h1`)** — axe lints static markup, the SR consumes the live tree with the toast overlay's `aria-hidden`/inert applied. axe-only would have shipped H4 + H9 as *passing*. Correct model = 3-tier: axe (names/ARIA) → AT-snapshot (`cat7-sr-tree.mjs`, landmarks/headings/operability context) → manual VoiceOver (flow). | ✅ written up in AUDIT_REPORT Cat 7 "Methodology limitation" callout |
