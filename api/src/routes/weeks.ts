...
router.post('/:id/start', authMiddleware, async (req: Request, res: Response) => {
  try {
    ...
    broadcastToUser(getUserId(req), 'accountability:updated', { type: 'week_start', targetId: id as string });
...
router.patch('/:id/plan', authMiddleware, async (req: Request, res: Response) => {
  try {
    ...
    if (data.plan && data.plan.trim() !== '') {
      broadcastToUser(getUserId(req), 'accountability:updated', { type: 'weekly_plan', targetId: id as string });
    }
