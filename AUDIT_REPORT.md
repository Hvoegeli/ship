...
**Method (read this first — it matters for how to weight these).** The seven sections above are *harness-measured* baselines (each reproducible via a committed `scripts/audit/catN-*.mjs`). The findings below come from a separate **full read-through of the repository** (backend security, data integrity, real-time/scaling, frontend, infra) — they are **diagnoses by code inspection**, not harness output. Each cites `file:line`; where a finding is independently reproducible by a one-off query/command that is noted. Every item below was **verified against source** before inclusion (same discipline as the Cat-6/7 instrument validations — claims that didn't hold were dropped). **No application code was modified** — this remains a Phase-1 diagnosis. Severity is re-ranked conservatively by us, not inherited from any tool.
...
- **S13 [Medium-High] No CI pipeline.** `.github/` is absent — there is **no automated type-check / test / lint gate** between a developer's machine and production; deploys are local shell scripts that ship `dist/` straight to Elastic Beanstalk. (Confirms the orientation's "zero CI".)
...
Severity-ranked synthesis across all 7 categories. Each row: the finding, its category, the **committed script that reproduces it** (re-run identically in Phase 2 for before/after), and the measurable Phase-2 lever where the deck specifies one. **`H#`/`M#`/`L#` rows are harness-measured; `S#` rows are the deep-static-review findings** (method + `file:line` in the Supplementary Findings section above) — they're diagnoses by code inspection, several independently reproducible by a one-off query/command as noted. Full methodology/evidence in the per-category sections above; raw in `docs/audit/raw/`.
