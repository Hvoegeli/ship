...
| Missing ARIA labels or roles | `aria-required-children` (critical) on main_docs & view_document (a role's required child structure is malformed) + `listitem` (serious, list markup not in a `<ul>/<ol>`). Login page lacks `main`/`nav` landmarks; app pages have both `main` and `nav` in the DOM. `lang=en`, unique `<title>` on all pages ✅. `<h1>` present in DOM on all pages — but see SR-tree row below: the h1 is **not exposed to the a11y tree** on authenticated pages (modal-overlay obscures the real outline). |
| **SR a11y-tree landmarks** (Q3-C; what an SR user can actually jump to) | login **0** · main_docs **0** · view_document **1** (`complementary:Document list`) · issues **1** (`main`) · my_week **0** · team_dir **0**. **4 of 6 pages expose zero landmarks** to the SR (no `banner` / `navigation` / `main` / `contentinfo`). SR users cannot navigate by region — they have to read top-to-bottom. WCAG 1.3.1 / 2.4.1. |
| **SR a11y-tree heading outline** (Q3-C; what an SR user outlines) | Every authenticated page exposes **exactly one** heading to the a11y tree: `h2 Action Items` — that is the *modal toast's* heading, not the page's. **No page-level `h1` reaches the SR on any authenticated page.** The DOM h1 the axe probe counts is masked by the toast overlay. SR users outlining by `h` key get one node per page. WCAG 1.3.1 / 2.4.6. |
...
| **Focus-trap modal — root cause of the H4 keyboard ceiling** (Q3-C Tab probe) | First 15 Tab landings on every authenticated page cycle through **only 3 buttons**: `POST STANDUP` (the "Post standup for Week 14" toast), `Got it`, `Close`. The toast opens on every authenticated page render and **traps focus** until dismissed. This is the *reason* `cat7-a11y.mjs` measured 3 distinct reachable elements out of hundreds. Login page Tab order is also inverted: lands on `Password` before `Email address`. |

**Verdict on the README's "Section 508 / WCAG 2.1 AA compliant" claim — CONTRADICTED.** Lighthouse scores are high (91–100) — the app's *automated-detectable* hygiene is good — but THREE independent tools (Lighthouse + axe + a11y-tree proxy) compound the evidence: **critical** `aria-required-children`, `listitem` structure, **15** color-contrast (WCAG 1.4.3 AA) failures; keyboard traversal reaching only ~3 of hundreds of interactive elements on every authenticated page (WCAG **2.1.1 Keyboard** / **2.4.3 Focus Order**); and — added by the SR-tree probe — **4 of 6 pages have zero landmarks** for SR navigation and **no page-level `h1` reaches the SR** on any authenticated page (only the modal toast's `h2`). A blanket "508 / WCAG 2.1 AA compliant" claim requires *zero* AA failures, full keyboard operability, AND a usable SR experience; none of those hold. So: **not supported**, despite the reassuring Lighthouse numbers — which is exactly why the PRD asks for keyboard + contrast + screen-reader testing beyond the score.

**Methodology limitation — why axe-core alone is *not* sufficient for screen-reader concerns (the reason Q3 ran C+B, not axe-only).** A natural objection is: *"screen-reader concerns are already covered by axe-core's name/role/structure rules (accessible names, ARIA validity, landmarks, heading order) — the things a screen reader actually surfaces — so a manual/SR-tree pass is redundant."* **Our own evidence refutes that for two of those four properties.** axe lints the **static DOM markup**; a screen reader consumes the **live accessibility tree** with overlays, `aria-hidden`/`inert`, and focus state already applied. The "Post standup for Week 14" toast is *correctly* coded in isolation (named buttons, valid ARIA — axe passes it), but at runtime it prunes the page's real landmarks and heading from the tree the SR receives. Head-to-head on the same 6 pages:

| Property | axe reported (`cat7-before.txt`) | Actual a11y tree (`cat7-sr-before.txt`) | |
|----------|----------------------------------|------------------------------------------|---|
...
| **Landmarks** | Probe 4: `main=true,nav=true` on all 5 authed pages | **4/6 pages expose ZERO landmarks** to the tree | ❌ **contradiction** |
| **Heading order** | Probe 4: `h1=1` on every page | every authed page exposes **only** the modal's `h2`; **no page `h1` reaches the SR** | ❌ **contradiction** |

So axe rated the landmark + heading structure of every authenticated page as clean, while the tree the SR actually navigates exposes one heading and (mostly) zero landmarks per page. **axe-only would have shipped this app's two highest-severity SR findings (H4 focus-trap, H9 landmark/heading erasure) reported as passing.** axe is also structurally blind to the focus-trap itself (runtime behavior, not a markup rule). The correct model is three-tier, not "automation vs. manual": **(1) axe** for accessible names + ARIA validity (static rules — strong); **(2) a runtime AT snapshot** (`getFullAXTree`, our `cat7-sr-tree.mjs`) for landmarks / heading outline / operability context that static linting cannot see; **(3) manual VoiceOver** for the one thing neither tool scores — whether the announcement *flow* is usable. Tier 2 is what caught everything axe missed here, reproducibly.

**Q3-B manual VoiceOver pass — status: blocked/deferred (Tier 3 above).** Plan was to walk the 3 highest-impact pages (`/login`, `/docs`, `/documents/<wiki-id>`) with macOS VoiceOver active (Cmd+F5), recording steps inline as `[VO-1] login: …`. The operator attempted this and hit blockers (recorded in RUNBOOK). Until a manual pass is recorded, the Q3 *qualitative-flow* layer (Tier 3) is **not** complete — but Tiers 1+2 are, so the PRD's "screen reader compatibility" line is **substantiated by reproducible automated AT inspection**, with the manual-flow check **explicitly flagged as a remaining gap** rather than overclaimed. Notably, a sighted operator finding VoiceOver navigation of the authenticated app frustrating is *itself* consistent with the H4/H9 findings (the focus trap and missing landmarks make the app genuinely hard to traverse non-visually).

**Weaknesses / opportunities (ranked):**
1. **High — keyboard navigation is broken on the authenticated app — *root cause now identified.*** The "Post standup for Week 14" toast modal opens on every authenticated page load and **traps focus** to its 3 buttons (POST STANDUP / Got it / Close). 60 Tabs reach only those 3 distinct focusable elements (vs **4/4** on the clean login control), despite 369–1017 interactive elements present. WCAG 2.1.1 / 2.4.3 — core Section 508. The single largest a11y gap; the SR-tree probe surfaced the cause that the axe probe could only measure as a ceiling. *Phase-2 lever:* dismiss/hide the toast from the focus cycle (and add a proper `aria-modal` + Escape-to-dismiss) — both `cat7-a11y.mjs` and `cat7-sr-tree.mjs` re-prove before/after.
2. **High — the README's 508 / WCAG 2.1 AA compliance claim is false as shipped.** Documentation overclaim with compliance risk for a `.treasury.gov` deployment; the audit's job was to test it, and three tools' evidence contradicts it (see verdict). *Phase-2 paths both now viable & measurable:* "+10 Lighthouse on the lowest page" (view_document **91** → ≥**100** is only +9 — so target the next-lowest or treat the +10 as fix-all on top-3) **or** "fix all Critical/Serious on the 3 most important pages" (cleaner given the low headroom — `cat7-a11y.mjs` + `cat7-lighthouse.mjs` + `cat7-sr-tree.mjs` prove it before/after).
3. **High — SR-navigability gap: 4/6 pages expose zero landmarks; no page-level `h1` reaches any authed page.** Screen-reader users navigate by landmark (`d`/`r` keys in VoiceOver/NVDA) and by heading (`h` key). The toast overlay obscures the real outline, and the underlying app pages don't appear to declare `main`/`nav` to the a11y tree even when they exist in the DOM. WCAG 1.3.1 / 2.4.1 / 2.4.6. *Phase-2 lever:* ensure each page renders a `<main>` containing the page's `<h1>`, and the toast is treated as an aside (`role="status"` if non-modal, `role="dialog" aria-modal` with proper labelling if modal). Re-prove via `cat7-sr-tree.mjs`.
...
6. **Low — login page missing `main`/`nav` landmarks** (region navigation) and Tab order lands on `Password` before `Email address` (Q3-C). Otherwise the cleanest page.
7. **Positive / scoping — baseline hygiene is solid.** `lang=en`, unique titles, app-page `main`+`nav` landmarks in DOM, 20–23 axe passes/page, and **0 unnamed interactives** across all 6 pages in the a11y tree (Q3-C). The gap is the focus-trap + landmark/heading surfacing + a few ARIA structure defects, not pervasive — a tractable, well-localized remediation target.
...
| H4 | Keyboard navigation broken on the authenticated app — **root cause: "Post standup for Week 14" toast traps focus to 3 buttons on every authed page load.** 60 Tabs reach only 3 distinct focusable elements (vs validated **4/4** login control) despite 369–1017 interactive elements. WCAG 2.1.1 / 2.4.3 — core Section 508. | 7 | `cat7-a11y.mjs` + `cat7-sr-tree.mjs` | Make toast non-focus-trapping (proper `role`/dismiss); fix focus order |
| H9 | **SR-navigability gap (a11y-tree proxy, Q3-C):** 4 of 6 audited pages expose **zero landmarks** to the accessibility tree (no `banner`/`nav`/`main`/`contentinfo` for SR jump-navigation); every authenticated page exposes **only one heading** to the tree (`h2 Action Items` — the modal's; no page-level `h1` reaches the SR). Screen-reader users navigating by landmark/heading get one stop per page. WCAG 1.3.1 / 2.4.1 / 2.4.6. | 7 | `cat7-sr-tree.mjs` | Render `<main>` with the page `<h1>`; demote toast to `role="status"` or proper modal; re-prove via SR-tree |
