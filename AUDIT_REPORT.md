...
**Q3-B manual VoiceOver pass — placeholder (run before final submission).** Operator: walk the 3 highest-impact pages (`/login`, `/docs`, `/documents/<wiki-id>`) with macOS VoiceOver active (Cmd+F5). Steps recorded inline as `[VO-1] login: …` etc. when run.  Until the manual pass is recorded, the Q3 evidence is **only the C-proxy above**; the PRD's "screen reader compatibility" line is **partially substantiated** — and we are explicit about that gap rather than overclaiming.
